...
def compute_recency_feature(last_used_at: datetime | None) -> float:
    """
    feature: recency
...
def compute_priority_feature(priority: int | None) -> float:
    """
    feature: priority
...
def _get_high_tags(baby_card_id: int | None, card_id: int | None) -> set[int]:
...
def filter_by_tag_semantic(
    selected_baby_card_id: int | None,
...
def filter_by_pos_transition(selected_pos: str | None, candidates: list[dict]) -> list[dict]:
    """
